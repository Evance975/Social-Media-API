...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict()) #converts all our entries into a dictionary instead of typing them manualy one by one incase we add another field in our model
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not allowed to perform the action")
